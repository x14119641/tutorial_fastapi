  ...
  <aside :class="['transition-width duration-300', sidebarOpen ? 'w-64' : 'w-20', darkMode ? 'bg-gray-800 text-white' : 'bg-gray-300 text-black']">
    <div class="p-4 flex justify-between">
      ...
      <button @click="toggleSidebar" class="focus:outline-none">
        <svg v-if="!sidebarOpen" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-6 h-6">
    ...
    <nav class="p-4">
      <ul>
        <li class="mb-4">
          <router-link to="/login" class="text-lg" :class="{ 'hidden': !sidebarOpen }">Login</router-link>
        </li>
        <li class="mb-4">
          <router-link to="/whoami" class="text-lg" :class="{ 'hidden': !sidebarOpen }">Profile</router-link>
        </li>
        <li>
          <router-link to="/token" class="text-lg" :class="{ 'hidden': !sidebarOpen }">Token</router-link>
        </li>
      </ul>
    </nav>
  </aside>
